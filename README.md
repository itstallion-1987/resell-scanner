...
### Сборка iOS без Mac (CI)

Swift нельзя собрать на Windows. Чтобы проверить компиляцию на настоящем macOS, в репозитории есть `.github/workflows/ci.yml`: при пуше в GitHub он на macOS-раннере ставит XcodeGen, генерирует проект, резолвит RevenueCat и собирает под симулятор **без подписи** (Apple Developer аккаунт не нужен). Для публичного репо — бесплатно. Это compile-check, не интерактив; «потыкать» приложение в Симуляторе можно через облачный Mac по часам (MacinCloud / AWS EC2 Mac).

Шаги: создать репозиторий на GitHub → `git remote add origin <url>` → `git push -u origin main` → вкладка **Actions** покажет результат сборки и ошибки компилятора, если они есть.
